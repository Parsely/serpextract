...
_country_codes = [country.alpha2.lower() for country in countries]
...
def _is_url_without_path_query_or_fragment(url_parts):
    ...
    """
    return (
        url_parts.path.strip("/") in ["", "search"]
        and url_parts.query == ""
    ...
    for engine_name, rule_group in iteritems(matomo_engines):
        ...
        for rule in rule_group:
            ...
            for i, domain in enumerate(rule["urls"]):
                ...
                _engines[domain] = SearchEngineParser(
                    engine_name,
                    defaults["extractor"],
                    defaults["link_macro"],
                    defaults["charsets"],
...
def _expand_country_codes(urls):
    ...
    expanded_urls = {
        url.format(country_code) for url in urls for country_code in _country_codes
    }
    expanded_urls.update(
        {
            url.format(second_level_domain + "." + cc_sub_domain)
            for url in urls
...
class SearchEngineParser(object):
    ...
    def __repr__(self):
        repr_fmt = (
            "SearchEngineParser(engine_name={!r}, "
...
def is_serp(referring_url, parser=None, use_naive_method=False):
    ...
    """
    res = extract(referring_url, parser=parser, use_naive_method=use_naive_method)
